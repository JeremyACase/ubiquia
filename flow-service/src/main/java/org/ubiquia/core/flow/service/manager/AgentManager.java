...
/**
 * This is a service that will manage data transforms for AMIGOS at runtime.
 */
@Component
public class AgentManager {
     ...
     */
    @Transactional
    public void tryDeployTransformsFor(final GraphDeployment graphDeployment)
        ...
        for (var agentEntity : graphEntity.getAgents()) {
            ...
            if (agent.getDataTransformType().equals(AgentType.POD)) {
                if (Objects.isNull(this.agentOperator)) {
                    ...
                    this.agentOperator.tryDeployTransform(agent);
                }
            } else {
                logger.info("...agent is not a POD transform; not deploying...");
            }
        }
