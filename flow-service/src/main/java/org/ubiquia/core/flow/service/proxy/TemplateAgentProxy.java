 ...
 * in an Ubiquia DAG is defined as a "template" transform, this service can be used to act
 * as a proxy for that transform.
...
public class TemplateAgentProxy {
    ...
    /**
     * This method will act as a "proxy" for a real data transform by generating fuzzy data
     * when an upstream event is sent. Put differently, this method allows an adapter to send
     * data to this service as if it were the actual data transform.
     *
     * @param flowEvent The event generated by the transform's adapter.
     ...
     */
    public void proxyAsAgentFor(FlowEvent flowEvent)
        throws JsonProcessingException,
        GenerationException,
        JsonGeneratorException {

        var eventTimes = flowEvent.getFlowEventTimes();
        eventTimes.setPayloadSentToTransformTime(OffsetDateTime.now());
        this.tryInitializeSchema(flowEvent.getAdapter().getId());
        var jsonSchema = this.cachedAdapterSchemas.get(flowEvent.getAdapter().getId());
