 ...
 */
@Service
public class QueueAdapterLogic implements InterfaceLogger {
    ...
    private static final Logger logger = LoggerFactory.getLogger(QueueAdapterLogic.class);
    ...
    public ResponseEntity<QueueAdapterEgress> popFor(final QueueAdapter adapter)
        ...
        ResponseEntity<QueueAdapterEgress> response = null;
        QueueAdapterEgress egress = null;
        var messages = this.inbox.tryQueryInboxMessagesFor(adapter);
        if (!messages.isEmpty()) {
