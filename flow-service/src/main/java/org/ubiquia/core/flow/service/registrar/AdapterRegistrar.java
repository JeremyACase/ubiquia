...
/**
 * This is a service that can be used to register adapters with AMIGOS.
 */
@Service
public class AdapterRegistrar {
    ...
    public List<Adapter> registerAdaptersFor(
        ...
        var adaptersAttachedToDataTransforms =
            this.tryRegisterAdaptersAttachedToDataTransforms(
                graphEntity,
                graphRegistration);

        var adaptersWithoutDataTransforms =
            this.tryRegisterAdaptersWithoutAgents(
                graphEntity,
                graphRegistration);

        adapterEntities.addAll(adaptersAttachedToDataTransforms);
        adapterEntities.addAll(adaptersWithoutDataTransforms);
    ...
    /**
     * Attempt to register any adapters for a graph that are explicitly attached to data
     * transforms.
     *
     ...
     * @return A list of newly-registered adapters.
     */
    private List<Adapter> tryRegisterAdaptersAttachedToDataTransforms(
        Graph graphEntity,
        final GraphDto graphRegistration)
        throws JsonProcessingException {
        ...
        for (var agent : graphRegistration.getAgents()) {
            ...
            if (Objects.nonNull(agent.getAdapter())) {
                ...
                var dataTransformMatch = graphEntity.getAgents()
                    .stream()
                    .filter(x -> x
                        .getAgentName()
    ...
    /**
     * Attempt to register adapters for a graph that are not attached to data transforms.
     *
     * @param graphEntity       The graph to register adapters for.
     * @param graphRegistration The object representing the graph registration.
     * @return A list of newly-registered adapters.
    ...
    private List<Adapter> tryRegisterAdaptersWithoutAgents(
        ...
        logger.info("...registering adapters without data transforms...");
