...
public class FlowEventDtoMapper extends GenericDtoMapper<FlowEvent, FlowEventDto> {
    ...
    private void setFlowEventTimes(final FlowEvent from, FlowEventDto to) {
        var flowEventTimes = new FlowEventTimes();
        flowEventTimes.setDataTransformResponseTime(from.getDataTransformResponseTime());
        flowEventTimes.setEgressResponseReceivedTime(from.getEgressResponseReceivedTime());
        flowEventTimes.setPayloadEgressedTime(from.getPayloadEgressedTime());
        flowEventTimes.setEventStartTime(from.getEventStartTime());
        flowEventTimes.setEventCompleteTime(from.getEventCompleteTime());
        flowEventTimes.setPollStartedTime(from.getPollStartedTime());
        flowEventTimes.setSentToOutboxTime(from.getSentToOutboxTime());
        to.setFlowEventTimes(flowEventTimes);
    }
}
