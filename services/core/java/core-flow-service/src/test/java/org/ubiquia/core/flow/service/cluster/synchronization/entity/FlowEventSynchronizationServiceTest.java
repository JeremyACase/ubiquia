...
public class FlowEventSynchronizationServiceTest {

    @MockBean
    private RestTemplate restTemplate;

    @Autowired
    private FlowEventSynchronizationService flowEventSynchronizationService;
    ...
    @Test
    public void assertSync_withNoFlowEvents_doesNotPost() {
        var agentEntity = this.agentRepository.findById(this.agentConfig.getId()).orElseThrow();

        this.flowEventSynchronizationService.sync(List.of("http://peer:8080"), agentEntity);

        verify(this.restTemplate, never()).postForEntity(anyString(), any(), any());
    }

    @Test
    public void assertSync_withFlowEventAndPeerSuccess_postsAndCreatesSyncEntity()
        throws Exception {

        this.createFlowEventChain();
        var agentEntity = this.agentRepository.findById(this.agentConfig.getId()).orElseThrow();
    ...
    @Test
    public void assertSync_withFlowEventAndPeerFailure_doesNotCreateSyncEntity()
        throws Exception {
