...
## [0.38.1] - 2026-06-17
### Fixed
- `common/java/model/ubiquia`: resolved all Google Java Style checkstyle violations (missing Javadoc on types and methods, `CustomImportOrder`, `AbbreviationAsWordInName`, `LineLength`, `OperatorWrap`, `SummaryJavadoc`, `EmptyLineSeparator`)
- `common/java/library/belief-state-libraries`: resolved all Google Java Style checkstyle violations in both main and test sources (missing Javadoc, `CustomImportOrder`, `AbbreviationAsWordInName` — `queryURL` → `queryUrl`, `getURL` → `getUrl`, `deleteURL` → `deleteUrl`, `assertPOSTsModel` → `assertPostsModel` — `VariableDeclarationUsageDistance`, `LineLength`, `OperatorWrap`, `SummaryJavadoc`, `EmptyLineSeparator`)
